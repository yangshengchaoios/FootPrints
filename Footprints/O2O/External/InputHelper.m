...
@interface InputHelper ()<UIGestureRecognizerDelegate>
...
- (void)initilize
{
    ...
    _defaultToolBarFrame = CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, toolBarHeight);
    _defaultMaskViewFrame = CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height);
    _toolBarFrameInMaskView = CGRectMake(0, [[UIScreen mainScreen] bounds].size.height - toolBarHeight, [[UIScreen mainScreen] bounds].size.width, toolBarHeight);
    
    _perMoveDistanceForInputField = 80;
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer{
    ...
    for (UIView *view in sortedInputFields) {
        if (CGRectContainsPoint(view.frame, [gestureRecognizer locationInView:_currentRootView])) {
            
            if (view != firstResponder && !view.hidden) {
                [view becomeFirstResponder];
                
                [self updateButtonEnabledStateForInputField:view];
            }
