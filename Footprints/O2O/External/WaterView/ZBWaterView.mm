...
- (void)waterScroll:(UIScrollView *)scrollView
{
    ...
    if (_visibleRange.length > 0) {
        ...
        CGFloat topOutHeight = (topOutPoint).y+[[_resHeightDic objectForKey:[NSNumber numberWithInt:(int)_visibleRange.location]] floatValue];
    ...
    if (_visibleRange.location >= 1) {
        ...
        if (topEntity.point.y>TopLineHeight) {//循环添加到移入点
            while (_visibleRange.location>= topEntity.index+1) {//be caution location is a non-negtive
                ...
                NSInteger location = _visibleRange.location;
                UIView *view = [self drawView:topInPoint index:(int)(location-1)];
...
- (void)initialize
{
    ...
    NSInteger numbers = [self.waterDataSource numberOfFlowViewInWaterView:self];
    ...
    //初始化视图
    CGFloat y = _topEntityVector.empty()?0:_topEntityVector[0].point.y;
    self.contentSize = CGSizeMake(self.frame.size.width, y+_waterInfo.topMargin);
    [self initView];
}
