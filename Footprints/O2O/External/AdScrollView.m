...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(adDidTap)];
...
- (void)setImageNameArray:(NSArray *)imageNameArray
{
    ...
    _pageControl.hidden = _imageNameArray.count<2;
    self.scrollEnabled = _imageNameArray.count>1;
...
- (void)setImageView:(UIImageView *)view withIndex:(NSInteger)index{
    
    
    NSInteger realIndex = index;
    if (index<0) {
        realIndex = self.imageNameArray.count-1;
    }else if (index>=self.imageNameArray.count) {
        realIndex = 0;
    }
    if (realIndex<self.imageNameArray.count && realIndex>=0) {
        [view sd_setImageWithURL:[NSURL URLWithString:self.imageNameArray[realIndex]] placeholderImage:kPlaceHolderImage];
    }
}
...
- (void)setCurrentImage:(NSInteger)currentImage{
    ...
    if (currentImage<0) {
        _currentImage = self.imageNameArray.count-1;
    }else if (currentImage>=self.imageNameArray.count){
        _currentImage = 0;
    }
    ...
    _pageControl.currentPage = _currentImage;
    
    [self setImageView:_leftImageView withIndex:self.currentImage-1];
    [self setImageView:_centerImageView withIndex:self.currentImage];
    [self setImageView:_rightImageView withIndex:self.currentImage+1];
}
...
- (void)adDidTap{
    ...
    if (self.didTapBlock) {
        self.didTapBlock(self.currentImage);
    }
}
@end
